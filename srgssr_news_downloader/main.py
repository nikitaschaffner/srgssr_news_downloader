...
class Window(QtWidgets.QMainWindow):
        ...
        if "status_label" in label_dict:
            if "text" in label_dict["status_label"]:
                ...
                if "color" in label_dict["status_label"]:
                    self.label_status_value.setStyleSheet(f"color: {label_dict['status_label']['color']};")
                else:
                    self.label_status_value.setStyleSheet("color: black")
            else:
                self.log.error("Error in text label change: 'status_label' was sent but no text information was given.")
        ...
        if "download_label" in label_dict:
            if "text" in label_dict["download_label"]:
                ...
                if "color" in label_dict["download_label"]:
                    self.label_download_value.setStyleSheet(f"color: {label_dict['download_label']['color']};")
                else:
                    self.label_download_value.setStyleSheet("color: black")
            else:
                self.log.error("Error in text label change: 'label_download_value' was sent but no text information was given.")
    ...
    def setup_config(self):
        ...
        try:
            ...
            dlg = confirmationDialog(self, "Standard Einstellungen Laden", 
...
class configWindow(QDialog):
    ...
    def setup_ui(self):
        ...
        self.business_unit_input.setCurrentText(self.config_helper.get_value("api", "business_unit"))
        self.update_cycle_input = QLineEdit(self.config_helper.get_value("api", "update_cycle"))
        ...
        # Audio file section
        self.filename_input = QLineEdit(self.config_helper.get_value("audio_file", "filename"))
        self.filepath_input = QLineEdit(self.config_helper.get_value("audio_file", "filepath"))
    ...
    def save_settings(self):
        ...
        self.config_helper.set_value("audio_file", "filename", self.filename_input.text())
        self.config_helper.set_value("audio_file", "filepath", self.filepath_input.text())
